...
public class ShortLineDetailsFileNamingPolicy implements FileNamingPolicy {
    @Override
    public String getFileName(List<RouteModel> routeModels) {
        if (routeModels.size() == 1) {
            return routeModels.get(0).getLine().getName() + "-" + routeModels.get(0).getName().substring(0,3);
        }
        StringBuilder fileName = new StringBuilder();
        for (int i = 0; i < routeModels.size(); i++) {
            RouteModel routeModel = routeModels.get(i);
            fileName.append(routeModel.getLine().getName()).append("-").append(routeModel.getName(), 0, 3);
